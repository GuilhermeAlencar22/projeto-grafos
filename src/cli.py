...
def _pares_variados_dijkstra_padrao(graph: Graph) -> list[tuple[str, str]]:
    ...
    _, niveis = bfs(graph, hub, return_levels=True)
    if niveis:
        ...
        _, n2 = bfs(graph, mais_longe, return_levels=True)
        if n2 and len(out) < 5:
...
def _pares_dijkstra_do_cli(
    ...
    if source and target:
        if source not in graph.adj or target not in graph.adj:
            raise ValueError(f"vertice fora do grafo: {source} ou {target}")
...
def _executar_check(
    ...
    ordem_bfs, _ = bfs(g2, fonte, return_levels=True)
...
def _executar_bfs(g2: Graph, args: argparse.Namespace, t0_total: float) -> None:
    ...
    resultado, niveis = bfs(g2, fonte_exec, return_levels=True)
...
def _executar_tres_fontes(g2: Graph, args: argparse.Namespace, t0_total: float) -> None:
    ...
    for s in fontes:
        ...
        ordem_b, niveis = bfs(g2, s, return_levels=True)
...
def _executar_dijkstra(g2: Graph, args: argparse.Namespace) -> None:
    ...
    for src, tgt in pares:
        ...
        if resultado is None or not resultado[1]:
            registros.append(
                {
                    "origem": src,
                    "destino": tgt,
